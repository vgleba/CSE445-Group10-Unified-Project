...
public sealed class GeoResolveService
{
    ...
    public GeoResolveService(GazetteerService gaz, IConfiguration cfg)
    {
        _gaz = gaz; _cfg = cfg;
    }
    ...
    // phrases like "NW of City", "пн-зх від Міста", "northwest of City"
    private static readonly Regex OfDir = new(@"(?ix)
    ...
    {
        var defR = double.TryParse(_cfg["GeoResolve:DefaultRadiusKm"], out var r) ? r : 8.0;
        var off = double.TryParse(_cfg["GeoResolve:DirectionOffsetKm"], out var o) ? o : 10.0;
