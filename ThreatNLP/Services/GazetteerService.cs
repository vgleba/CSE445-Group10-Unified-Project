...
public sealed class GazetteerService
{
    ...
    public GazetteerService(IConfiguration cfg)
    {
        _path = Path.Combine(AppContext.BaseDirectory,
            cfg.GetSection("GeoResolve")["GazetteerPath"] ?? "App_Data/gazetteer.json");
    }
    ...
    public IReadOnlyList<GazetteerPlace> All()
    {
        ...
        lock (_lock)
        {
            ...
            var list = JsonSerializer.Deserialize<List<GazetteerPlace>>(json,
    ...
    public GazetteerPlace? FindBest(string text)
    {
        ...
        return bestScore >= 80 ? best : null; // require reasonably good match
    }
